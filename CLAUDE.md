...
- `FallbackChain` and `Throttle` are themselves `LLMAdapter`s (composable decorators). The NaN cloud engine = `Throttle(FallbackChain(mimo-v2.5 → qwen3.6 → gemma4))`, exposed as a single `nan-cloud` model: ordered failover on availability/quota errors + a concurrency semaphore. See ADR-009.
