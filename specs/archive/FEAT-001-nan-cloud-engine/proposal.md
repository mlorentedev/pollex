...
<!-- from 10_projects/pollex/11-tasks.md: FEAT-001 — add a nan.builders OpenAI-compatible adapter exposed as "NaN Cloud (auto)" alongside the Jetson llama.cpp engine, backed by a 3-model fallback chain (mimo-v2.5 → qwen3.6 → gemma4). -->
...
3. `GET /api/models` lists a **single** `"NaN Cloud (auto)"` entry (model id e.g. `nan-cloud`) when the NaN key is configured, coexisting with the Jetson llama.cpp model(s). The extension dropdown shows it; the user chooses the engine via the existing `model_id` request mechanism.
