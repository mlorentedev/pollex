...
func TestLlamaCppAdapterPolish(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := llamaCppChatResponse{
			Choices: []llamaCppChoice{
				{Message: llamaCppMessage{Role: "assistant", Content: "I went to the store."}},
			},
		}
...
func TestLlamaCppAdapterPolishServerError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestLlamaCppAdapterPolishEmptyChoices(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := llamaCppChatResponse{Choices: []llamaCppChoice{}}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}))
	defer srv.Close()
...
func TestLlamaCppAdapterPolishContextCancel(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestLlamaCppAdapterAvailable(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestLlamaCppAdapterNotAvailable(t *testing.T) {
	a := &LlamaCppAdapter{
...
func TestLlamaCppAdapterName(t *testing.T) {
	a := &LlamaCppAdapter{Model: "qwen2.5-1.5b"}
	want := "llama.cpp (qwen2.5-1.5b)"
	if a.Name() != want {
